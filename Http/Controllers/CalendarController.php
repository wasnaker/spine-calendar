...
class CalendarController extends Controller
{
    ...
    {
        $query = Calendar::query()->where('user_id', $request->user()->id);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $entity = Calendar::create($validated + ['user_id' => $request->user()->id]);
    ...
    {
        $entity = Calendar::where('user_id', request()->user()->id)->find($id);
    ...
    {
        if (! Calendar::where('user_id', $request->user()->id)->find($id)) {
            return response()->json(['message' => 'Calendar not found'], 404);
        }

        $logs = $this->activityLog
            ->query()
            ->where('subject_type', Calendar::class)
            ->where('subject_id', $id)
